...
@Component({
  selector: 'app-search-voyage',
  templateUrl: './search-voyage.page.html',
  styleUrls: ['./search-voyage.page.scss'],
})
export class SearchVoyagePage implements OnInit {
   ...
   /* ionViewDidEnter() {
    setTimeout(() => {
     ...
     getItems(ev: any) {
         ...
         // if the value is an empty string don't filter the items //voila
         if (val && val.trim() !== '') {
             this.isItemAvailable = true;
